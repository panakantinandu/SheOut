...
interface CustomerProfileRepository extends JpaRepository<CustomerProfileEntity, UUID> {
    ...
    /** Addresses only, for an announcement - see CustomerProfileApi.findEmailAddresses. */
    @Query("select p.email from CustomerProfileEntity p where p.email is not null and p.email <> '' order by p.accountId")
